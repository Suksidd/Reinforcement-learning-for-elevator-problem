...
class ElevatorEnv(object):
    ...
    def __init__(self, elevators = 2, floors = 5, people_on_each_floor = 3): #initialize the values
        self.floors = floors
    ...
    def elevatorFunction(self,  floor):  # floor setting
    ...
    def isTerminalState(self): #terminate the loop 
        count = 0
    ...
    def moveEle(self, newLoc):#move the elevator
        if self.doorstate == 0:
    ...
    def setFloors(self): #used to pick up person and then depoist them back to the requried request floora
    ...
    def step(self, action): #used to define the reward function for the respective movement
    ...
    def render(self):#elevator action representation 
        print('----------------------')
    ...
    def reset(self): #reset in case of error
        self.ele_map = [['_' for i in range(self.elevator_capacity)] for i in range(self.floors)]
        self.floor_map = [['_' for i in range(self.people_on_each_floor)] for i in range(self.floors)]
        self.loc_elevator = 0
        self.doorstate = 0
...
def maxAction(Q, state, actions): #get the actions for respective states
    values = np.array([Q[state,a] for a in actions])
    action = np.argmax(values)
    return actions[action]

def getRandomAction(): #randomize actions
    actions = ['U', 'C', 'O', 'D']
    return random.choice(actions)

if __name__ == '__main__':
    # model hyperparameters
    env = ElevatorEnv(elevators = 2, floors = 10, people_on_each_floor = 3)
    ALPHA = 1 # setp parameter
    GAMMA = 1# discount parameter
    EPS = 1
